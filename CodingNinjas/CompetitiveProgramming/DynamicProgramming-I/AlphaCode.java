...
public class AlphaCode {
  ...
  public static long solveBottomUp(String input) {
    ...
    for (int i = 2; i <= input.length(); i++) {
      ...
      int digit = Character.getNumericValue(input.charAt(i - 2)) * 10 + Character.getNumericValue(input.charAt(i - 1));
  ...
  public static int solve(String input, int index, int[] dp) {
    ...
    if (index + 1 < input.length()) {
      int optionTwo = Character.getNumericValue(input.charAt(index)) * 10

...
public class WarmReception {
	...
	public static int findMaxOverlapping(List<Interval> intervals) {
		...
		System.out.println("Overlapping interval: " + intervals.get(maxOverlapping).start + " "
				+ intervals.get(maxOverlapping).end);
...
class Interval {
	...
	public boolean doesOverlap(Interval i) {
		if (this.start >= i.start && this.start < i.end || this.end <= i.end && this.end > i.start

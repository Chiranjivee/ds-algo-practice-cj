...
public class ConnectingTheDots {
    ...
    public void dfsUtil(
        char[][] graph, NodeStatus[][] visited,
        int startX, int startY, int [] count, char currentChar) {
    ...
    public boolean isValid(int x, int y, NodeStatus[][] visited, char[][] graph, char currentChar) {
        if (x < 0 || x >= visited.length || 
            ...
            visited[x][y] == NodeStatus.VISITED ||
            currentChar != graph[x][y] ||
            visited[x][y] == NodeStatus.IS_BEING_VISITED) {
            return false;
        }
        return true;
    }

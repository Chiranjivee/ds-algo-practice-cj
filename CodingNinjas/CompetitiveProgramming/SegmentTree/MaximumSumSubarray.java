...
public class MaximumSumSubarray {
    ...
    public static void buildTree(int[] arr, TreeNode[] tree, int start, int end, int treeNodeIdx) {
        ...
        tree[treeNodeIdx].maxSum = Math.max(Math.max(left.maxSum, right.maxSum),
                Math.max(left.bestSuffixSum + right.bestPrefixSum,
                        Math.max(left.sum + right.bestPrefixSum, right.sum + left.bestSuffixSum)));
    }
    ...
    public static TreeNode query(TreeNode[] tree, int start, int end, int left, int right, int treeNodeIdx) {
        ...
        res.maxSum = Math.max(Math.max(option1.maxSum, option2.maxSum),
                Math.max(option1.bestSuffixSum + option2.bestPrefixSum,
                        Math.max(option1.sum + option2.bestPrefixSum, option2.sum + option1.bestSuffixSum)));
        return res;
    }
}
